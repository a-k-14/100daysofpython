...
task_option_menu = ctk.CTkComboBox(app, values=["100 days of python ", "IIMBAA", "PDS", "PYL", "100 days of python", "IIMBAA", "PDS", "PYL", "100 days of python 100 days of python", "IIMBAA", "PDS", "PYL", "PYL"], command=option_menu_callback, state="readonly")
...
timer_entry = ctk.CTkEntry(app, textvariable=timer_var, font=("Segoe UI Symbol", 35, "bold"), justify="center", text_color="#9e9e9e", state="disabled")
timer_entry.grid(row=2, column=1, columnspan=3, sticky="we", padx=10, pady=10)

notes_entry = ctk.CTkTextbox(app, text_color="#f2f2f2", height=65, width=180, font=('Segoe UI', 14), border_color="#4c5154", border_width=1)
...
def start_timer_action():
    ...
    if not running:
        ...
        if paused_time > 0: total_paused_duration += (time.time() - paused_time); paused_time = 0
        else: start_time = time.time(); total_paused_duration = 0
        running = True; start_btn.configure(text="⏸", fg_color="#FBC02D"); update_timer()
        print("Timer Started/Resumed")
    else:
        running = False; paused_time = time.time(); start_btn.configure(text="▶", fg_color="#085bbe")
        if timer_id: app.after_cancel(timer_id)
        print("Timer Paused")
def stop_timer_action():
    ...
    if running or paused_time > 0:
        ...
        running = False; start_time = 0; paused_time = 0; total_paused_duration = 0; timer_id = None; timer_var.set("00:00:00")
...
start_btn= ctk.CTkButton(app, text="▶", command=start_timer_action, cursor="hand2", font=("Segoe UI Symbol", 16, "bold"), width=40, fg_color="#085bbe")
start_btn.grid(row=4, column=1, padx=10, pady=10, sticky="we")

stop_btn = ctk.CTkButton(app, text="⏹", cursor="hand2", font=("Segoe UI Symbol", 16), width=40, command=stop_timer_action, fg_color="#C62828")
stop_btn.grid(row=4, column=2, sticky="we")

reset_btn = ctk.CTkButton(app, text="Reset", cursor="hand2", command=reset_timer_action, width=60, fg_color="#242424", border_color="#414449", border_width=1)
